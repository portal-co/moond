...
Summary
- GO is the error/restart instruction: transfers control to fixed restart address (typically 0o4000) to begin restart or recovery routines.

Pseudocode

```c
// GO: Transfer to restart/error handler (Block-2)
// See ref/definitions/STD2.md for canonical subinstruction patterns
void GO(void) {
    // Fixed restart address (AGC standard: octal 4000)
    const uint16_t restart_addr = 0o4000;
    
    // Branch to restart handler
    Z = restart_addr;
    uint16_t restart_instr = memory[Z];
    SQ = extract_order_code(restart_instr);
}
```
